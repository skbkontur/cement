...
{
    private readonly Dictionary<string, bool> depConfigurationExistsCache = new();

    private readonly ILogger logger;
    private readonly ConsoleWriter consoleWriter;
    private readonly BuildHelper buildHelper;

    public BuildPreparer(ILogger<BuildPreparer> logger, ConsoleWriter consoleWriter, BuildHelper buildHelper)
    {
        this.logger = logger;
    ...
    public static BuildPreparer Shared { get; } = new(LogManager.GetLogger<BuildPreparer>(), ConsoleWriter.Shared, BuildHelper.Shared);
    ...
    private void TopSort(Dep dep, Dictionary<Dep, List<Dep>> graph, ISet<Dep> visitedConfigurations, List<Dep> processingConfigs,
                         List<Dep> result, bool printCycle)
    {
    ...
    private void Dfs(Dep dep, Dictionary<Dep, List<Dep>> graph, HashSet<Dep> visitedConfigurations)
    {
        ...
        var currentDeps = new DepsParser(
            consoleWriter, DepsValidatorFactory.Shared,
    ...
    private string GetCurrentCommitHash(string moduleName)
    {
        try
        {
            ...
            var repo = new GitRepository(gitRepositoryLogger, consoleWriter, buildHelper, shellRunner, repoPath, moduleName, workspace);
