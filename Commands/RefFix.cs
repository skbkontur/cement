...
namespace Commands
{
    public class RefFix : Command
    {
        ...
        private void Fix(string project, string reference)
        {
            ...
            var withSameName = installFiles.Where(file => Path.GetFileName(file).Equals(Path.GetFileName(reference)))
                .ToList();
            if (!withSameName.Any())
                withSameName = GetAllInstallFilesFromTree()
                    .Where(file => Path.GetFileName(file).Equals(Path.GetFileName(reference))).ToList();
        ...
        private List<string> GetAllInstallFiles(string module)
        {
            ...
            var result = configs.Select(config => Yaml.InstallParser(module).Get(config))
                .SelectMany(parser => parser.Artifacts);
            ...
            var configsWithArtifact =
                configs.Where(c =>
                    Yaml.InstallParser(moduleDep).GetAllInstallFilesFromConfig(c)
                        .Select(file => Path.Combine(moduleDep, file))
                        .Any(file => Path.GetFullPath(file) == Path.GetFullPath(reference))).ToList();

            var toAdd = DepsPatcherProject.GetSmallerCementConfigs(Path.Combine(Helper.CurrentWorkspace, moduleDep),
                configsWithArtifact);
            foreach (var configDep in toAdd)
            {
                DepsPatcherProject.PatchDepsForProject(Directory.GetCurrentDirectory(),
                    new Dep(moduleDep, null, configDep), project);
            }
        }
