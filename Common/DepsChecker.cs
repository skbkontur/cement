...
namespace Common
{
        ...
        public CheckDepsResult GetCheckDepsResult()
        {
            ...
            foreach (var bulid in buildData)
            {
                ...
                var refs = files.SelectMany(file =>
                    vsParser.GetReferencesFromCsproj(file).Select(reference => reference.Replace('/', '\\'))
                        .Select(r => new ReferenceWithCsproj(r, file)));
    ...
    public class DepsReferencesCollector
    {
        ...
        {
            return Yaml.SettingsParser(dep.Name).Get().IsContentModule ||
                   Yaml.BuildParser(dep.Name).Get(dep.Configuration).All(t => t.Target == "None");
        }
    }
