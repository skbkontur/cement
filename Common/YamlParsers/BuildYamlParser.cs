...
namespace Common.YamlParsers
{
    public class BuildYamlParser : ConfigurationYamlParser
    {
        ...
        private void ProcessBuildSection(string configName, Dictionary<string, object> buildSection,
            List<Dictionary<string, object>> buildSections)
        {
            ...
            if ((!buildSection.ContainsKey("configuration") ||
                 string.IsNullOrEmpty((string) buildSection["configuration"]))
        ...
        private List<Dictionary<string, object>> GetBuildSectionsFromConfig(string configuration)
        {
            ...
            return list.Cast<Dictionary<object, object>>()
                .Select(d => d.ToDictionary(kvp => (string) kvp.Key, kvp => kvp.Value)).ToList();
        }
    }
}
