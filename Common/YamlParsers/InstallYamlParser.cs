...
namespace Common.YamlParsers
{
    public class InstallYamlParser : ConfigurationYamlParser
    {
        ...
        public InstallData Get(string configuration = null)
        {
            ...
            result.ExternalModules.AddRange(
                defaultInstallContent.ExternalModules.Where(r => !result.ExternalModules.Contains(r)));
        ...
        private InstallData GetInstallContent(string configuration)
        {
            ...
            result.MainConfigBuildFiles.AddRange(GetAllInstallFilesFromConfig(configuration)
                .Where(r => !r.StartsWith("module ")));
            while (configQueue.Count > 0)
            {
                ...
                result.Artifacts.AddRange(GetInstallSectionFromConfig(currentConfig, "artifacts")
                    .Where(r => !r.StartsWith("module ")));
                result.Artifacts.AddRange(GetInstallSectionFromConfig(currentConfig, "artefacts")
                    .Where(r => !r.StartsWith("module ")));
