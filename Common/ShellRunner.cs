        ...
        public int RunOnce(string commandWithArguments, string workingDirectory, TimeSpan timeout)
        {
            ...
            using (process = Process.Start(startInfo))
            {
                try
                {
                    ...
                    if (!threadOutput.Join(timeout) || !threadErrors.Join(timeout) ||
                        !process.WaitForExit((int) timeout.TotalMilliseconds))
                    {
                        ...
                        var message = string.Format("Running timeout at {2} for command {0} in {1}",
                            commandWithArguments, workingDirectory, timeout);
                        Errors += message;
