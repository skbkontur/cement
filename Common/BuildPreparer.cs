...
namespace Common
{
    public class BuildPreparer
    {
        ...
        public void GetModulesOrder(string moduleName, string configuration, out List<Dep> topSortedVertices,
            out List<Dep> updatedModules, out Dictionary<string, string> currentCommitHashes)
        {
        ...
        private static Dictionary<Dep, List<Dep>> EraseExtraChildren(Dictionary<Dep, List<Dep>> configsGraph)
        {
            ...
            foreach (var parrent in vertices)
            {
                ...
                var hierarchyManager =
                    new ConfigurationManager(parrent.Name, vertices.Where(v => v.Name == parrent.Name).ToArray());
                ...
                foreach (var childConfig in childrenConfigurations)
                {
                    ...
                    try
                    {
                        GetTopologicallySortedGraph(configsGraph2, parrent.Name, parrent.Configuration,
                            printCycle: false);
        ...
        private static Dictionary<Dep, List<Dep>> EraseChild(Dictionary<Dep, List<Dep>> configsGraph, Dep child,
            Dep parrent)
        {
        ...
        public static List<Dep> GetTopologicallySortedGraph(Dictionary<Dep, List<Dep>> graph, string root,
            string config, bool printCycle = true)
        {
        ...
        private static void TopSort(Dep dep, Dictionary<Dep, List<Dep>> graph, ISet<Dep> visitedConfigurations,
            List<Dep> processingConfigs, List<Dep> result, bool printCycle)
        {
        ...
        private static void Dfs(Dep dep, Dictionary<Dep, List<Dep>> graph, HashSet<Dep> visitedConfigurations)
        {
            ...
            var currentDeps =
                new DepsParser(Path.Combine(Helper.CurrentWorkspace, dep.Name)).Get(dep.Configuration).Deps ??
                new List<Dep>();
