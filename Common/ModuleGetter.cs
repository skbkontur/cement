...
namespace Common
{
    public class ModuleGetter
    {
        ...
        private static readonly ILog Log =
            new PrefixAppender("MODULE-GETTER", LogManager.GetLogger(typeof(ModuleGetter)));
        ...
        {
            if (depsPool.Any())
                Log.Info("Parallel update-deps iteration: " + depsPool
                             .Select(d => d.Dep.ToString() + "(" + d.ParentModule + ")")
                             .Aggregate((a, b) => a + " " + b));
            try
            {
        ...
        private string GetPrintString(int longestModuleNameLength, string moduleName, string treeishInfo,
            string commitInfo)
        {
        ...
        private void PrintProcessedModuleInfo(Dep dep, GetInfo getInfo, string treeish)
        {
            ...
            var name = dep.Name + (dep.Configuration == null || dep.Configuration.Equals("full-build")
                           ? ""
                           : Helper.ConfigurationDelimiter + dep.Configuration);
            var outputTreeish = getInfo.Forced && !treeish.Equals("master")
                ? treeish + " *forced"
                : (dep.Treeish ?? "master");
        ...
        private void GetTreeish(GitRepository repo, Dep dep, string force, string treeish, GetInfo getInfo)
        {
            ...
            if (hasRemoteBranch && repo.RemoteCommitHashAtBranch(treeish).Equals(oldSha) &&
                repo.CurrentLocalTreeish().Value.Equals(treeish))
            {
                return;
            }
        ...
        private LocalChangesAction DefineLocalChangesPolicy(GitRepository repo, string localSha, string remoteSha)
        {
            ...
            switch (userLocalChangesPolicy)
            {
                ...
                case LocalChangesPolicy.Interactive:
                    Console.WriteLine(
                        "What do you want to do? enter 'f' for saving local changes / 'r' for resetting local changes(git clean & git reset) / 'p' for pull anyway :\n");
