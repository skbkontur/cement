...
namespace Common
{
    public class ModuleGetter
    {
        ...
        private void ProcessDeps(string force, List<DepWithParent> depsPool)
        {
            ...
            try
            {
                Parallel.ForEach(depsPool.Where(d => d.Dep.Name != rootModule.Name), new ParallelOptions {MaxDegreeOfParallelism = Helper.MaxDegreeOfParallelism}, d => GetModule(d.Dep, force));
            }
