...
namespace Common
{
    public class VisualStudioProjectParser
    {
        ...
        private bool IsReferenceToCementModule(string reference, string csprojDir)
        {
            try
            {
                ...
                if (idx == splitedRef.Count())
                    throw new CementException("Something went wrong. Really bad HintPath " + reference + " in " +
                                              csprojDir);
        ...
        public IEnumerable<string> GetCsprojList(string configName)
        {
            ...
            return guidSetForConfig.Where(guid => guidToCsprojDict.ContainsKey(guid))
                .Select(guid => guidToCsprojDict[guid]).ToList();
        }
        ...
        private IEnumerable<string> GetGuidSetForConfig(string configName)
        {
            ...
            foreach (
                var line in
                solutionContent
                    .Where(l => !string.IsNullOrEmpty(l) && l.ToLower().Contains($".{configName.ToLower()}|") &&
                                l.ToLower().Contains(".build.0"))
            )
