...
namespace Common
{
    public class DepsPatcher
    {
        ...
        {
            var parser = new DepsYamlParser(new FileInfo(Directory.GetParent(yamlPath).FullName));
            var hadInSameSection = parser.GetDepsFromConfig(patchConfiguration).Deps.Where(d => d.Name == patchDep.Name)
                .Distinct().ToList();
        ...
        {
            var parser = new DepsYamlParser(new FileInfo(Directory.GetParent(yamlPath).FullName));
            var hadDepsInThisSectionAndParrents =
                parser.Get(patchConfiguration).Deps.Where(d => d.Name == patchDep.Name).ToList();
        ...
        private void FixChild(Dep currentParrentDep, string childConfiguration)
        {
            ...
            var parser = new DepsYamlParser(new FileInfo(Directory.GetParent(yamlPath).FullName));
            var hadInSameSection = parser.GetDepsFromConfig(childConfiguration).Deps.Where(d => d.Name == patchDep.Name)
                .ToList();
            if (!hadInSameSection.Any())
                return;
            if (hadInSameSection.Count > 1)
        ...
        private Dep FindLca(Dep dep1, Dep dep2)
        {
            ...
            var lowestAncestor = commonAncestorsList.Where(c =>
                !new ConfigurationManager(commonAncestorsList.Select(cc => new Dep(null, null, cc)), configParser)
                    .ProcessedChildrenConfigurations(new Dep(null, null, c)).Any()).ToList();
        ...
        private void RemoveDepLine(string configuration, Dep dep, bool isOn)
        {
            ...
            if (depsIndex == -1)
                return;
            for (int index = depsIndex + 1;
                index < lines.Count && lines[index].StartsWith(GetSpacesStart(lines, depsIndex));
                index++)
            {
        ...
        private void AddDepLine(string configuration, Dep dep, bool isOn)
        {
            ...
            for (int index = configIndex + 1;
                index < lines.Count && (lines[index].Length == 0 || lines[index].StartsWith(" "));
                index++)
                if (lines[index].EndsWith("deps:"))
                {
                    depsIndex = index;
    ...
    public static class DepsPatcherProject
    {
        ...
        private static readonly HashSet<KeyValuePair<string, string>> patchedDeps =
            new HashSet<KeyValuePair<string, string>>();

        private static void PatchDeps(string currentModuleFullPath, List<string> modulesToInsert,
            List<string> cementConfigs)
        {
                ...
                foreach (var module in modulesToInsert)
                {
                    ...
                    catch (Exception exception)
                    {
                        ConsoleWriter.WriteError(
                            $"Fail adding {dep} to {moduleName}/{config} deps:\n\t{exception.Message}");
        ...
        public static List<string> GetSmallerCementConfigs(string modulePath, List<string> usedCementConfigs)
        {
            ...
            var configManager =
                new ConfigurationManager(usedCementConfigs.Select(c => new Dep("", null, c)), configParser);
