    ...
    public class TestBuildDepsOrder
    {
        ...
        public void TestConfigGraph()
        {
            using (var env = new TestEnvironment())
            {
                env.CreateRepo("A", new Dictionary<string, DepsContent>
                {
                    {
                        "full-build *default",
                        new DepsContent(null, new List<Dep> {new Dep("B"), new Dep("C", null, "client")})
                    }
                });
                ...
                Assert.AreEqual(new[] {new Dep("B", null, "full-build"), new Dep("C/client")},
                    result[new Dep("A", null, "full-build")].ToArray());
                Assert.AreEqual(new[] {new Dep("D", null, "full-build")},
                    result[new Dep("B", null, "full-build")].ToArray());
        ...
        public void TestRelaxNesting()
        {
            using (var env = new TestEnvironment())
            {
                ...
                new BuildPreparer(Log).GetModulesOrder("A", null, out topSortedDeps, out modulesToUpdate,
                    out currentCommitHashes);
                ...
                {
                    {
                        ...
                        new DepsContent(null,
                            new List<Dep> {new Dep("C1"), new Dep("C2"), new Dep("C3"), new Dep("P1")})
                    }
                });
