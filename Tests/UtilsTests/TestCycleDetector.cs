    ...
    public class TestCycleDetector
    {
        ...
        public void TestSimpleCycle()
        {
            using (var env = new TestEnvironment())
            {
                ...
                {
                    {"full-build", new DepsContent(null, new List<Dep> {new Dep("A")})}
                });
        ...
        public void TestLongCycle()
        {
            using (var env = new TestEnvironment())
            {
                ...
                {
                    {"full-build", new DepsContent(null, new List<Dep> {new Dep("E")})}
                });
                env.CreateRepo("E", new Dictionary<string, DepsContent>
                {
                    {"full-build", new DepsContent(null, new List<Dep> {new Dep("A")})}
                });
                Helper.SetWorkspace(env.RemoteWorkspace);
                ...
                Assert.AreEqual(
                    new[]
                    {
                        "A/full-build", "B/full-build", "C/full-build", "D/full-build", "E/full-build", "A/full-build"
                    }, cycle.ToArray());
            }
        }
        ...
        public void TestCycleDontHaveSourceModule()
        {
            using (var env = new TestEnvironment())
            {
                ...
                Helper.SetWorkspace(env.RemoteWorkspace);
                var cycle = CycleDetector.TryFindCycle("A");
                Assert.AreEqual(new[] {"A/full-build", "B/full-build", "C/full-build", "D/full-build", "B/full-build"},
                    cycle.ToArray());
            }
        }
        ...
        public void TestCycleWithDefaultConfig()
        {
            using (var env = new TestEnvironment())
            {
                ...
                {
                    {"full-build", new DepsContent(null, new List<Dep> {new Dep("B")})},
                    {"client *default", new DepsContent(null, new List<Dep> {new Dep("B")})}
                });
                env.CreateRepo("B", new Dictionary<string, DepsContent>
                {
                    {"full-build", new DepsContent(null, new List<Dep> {new Dep("A", null, "full-build")})}
                });
                Helper.SetWorkspace(env.RemoteWorkspace);
                var cycle = CycleDetector.TryFindCycle("A");
        ...
        public void TestCycleWithPredperiod()
        {
            using (var env = new TestEnvironment())
            {
                ...
                Helper.SetWorkspace(env.RemoteWorkspace);
                var cycle = CycleDetector.TryFindCycle("A");
                Assert.AreEqual(new[] {"A/full-build", "B/full-build", "C/full-build", "B/full-build"},
                    cycle.ToArray());
            }
        }
        ...
        public void TestCycleWithTwoWays()
        {
            using (var env = new TestEnvironment())
            {
                ...
                Helper.SetWorkspace(env.RemoteWorkspace);
                var cycle = CycleDetector.TryFindCycle("A");
                Assert.AreEqual(new[] {"A/full-build", "C/full-build", "D/full-build", "C/full-build"},
                    cycle.ToArray());
            }
        }
    }
