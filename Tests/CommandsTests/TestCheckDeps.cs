...
full-build:
  ...
  deps:
    B");
                ...
                Assert.AreEqual(new[] {"B\\bin\\Release\\B.dll"},
                    depsReferences.FoundReferences.First().MainConfigBuildFiles);
            }
        }
  ...
  deps:
    B");
                ...
                var depsReferences =
                    new DepsReferencesCollector(Path.Combine(tempDir.Path, "A"), null).GetRefsFromDeps();
                Assert.AreEqual(new[] {"C"}, depsReferences.NotFoundInstallSection);
                Assert.AreEqual(new[] {"B\\bin\\Release\\B.dll", "B\\bin\\Release\\B.Client.dll"},
                    depsReferences.FoundReferences.First().BuildFiles);
                Assert.AreEqual(new[] {"B\\bin\\Release\\B.dll"},
                    depsReferences.FoundReferences.First().MainConfigBuildFiles);
            }
        }
